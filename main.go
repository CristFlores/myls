...
func printList(fs []file, nRecords int) {
	for _, file := range fs[:nRecords] {
		...
		fmt.Printf("%s %s %s %10d %s %s %s %s %s\n", file.mode, file.userName, file.groupName, file.size, file.modificationTime.Format(time.DateTime), style.icon, setcolor(file.name, style.color), style.symbol, markHidden(file.isHidden))
	}
}
